...
const VALID_FILES = ["requirements.md", "design.md", "tasks.md"] as const;
type SpecFile = (typeof VALID_FILES)[number];
...
type TaskItem = { line: number; text: string; done: boolean };
type HookMeta = { name: string; event: string; description: string; file: string };
...
function filePath(name: SpecFile): string {
...
function readFile(name: SpecFile): string {
  const p = filePath(name);
  return fs.existsSync(p) ? fs.readFileSync(p, "utf-8") : "";
}

function writeFile(name: SpecFile, content: string): void {
  ensureDir();
...
function parseTasks(content: string): TaskItem[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m) tasks.push({ line: i, text: m[2].trim(), done: m[1].toLowerCase() === "x" });
  }
  return tasks;
}

function fileMetadata(name: SpecFile) {
  const p = filePath(name);
  if (!fs.existsSync(p)) return { exists: false, size: 0, tasks: 0, done: 0 };
...
function hookFilePath(name: string): string {
  return path.join(hooksDir(), `${name.replace(/[^a-zA-Z0-9_-]/g, "_")}.md`);
}

const server = new Server(
  { name: "spec-driver-mcp", version: "0.3.0" },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case "init-spec": {
      const { projectName, specType, workflowType: rawWf } = args as {
        projectName: string; specType?: string; workflowType?: string;
      };
      ...
      const hookSamples: { name: string; event: HookEvent; desc: string; body: string }[] = [
        {
    ...
    case "write-spec-file": {
      const { file, content } = args as { file: SpecFile; content: string };
    ...
    case "read-spec-file": {
      const { file } = args as { file: SpecFile };
      const content = readFile(file);
      if (!content) return { content: [{ type: "text", text: `${file} is empty or does not exist yet.` }] };
    ...
    case "list-spec-files": {
      ...
      const lines = [
        ...
        "",
        ...meta.map((m) =>
          m.exists ? `  ${m.name.padEnd(18)} ${m.tasks} tasks (${m.done} done, ${m.size} chars)` : `  ${m.name.padEnd(18)} (not yet created)`),
        "",
    ...
    case "update-task": {
      const { match, done, line } = args as { match?: string; done: boolean; line?: number };
      const content = readFile("tasks.md");
      if (!content) return { content: [{ type: "text", text: "tasks.md not found." }] };
      const lines = content.split("\n");
      const tasks = parseTasks(content);
      let targetLine = -1;

      if (line !== undefined) targetLine = line;
      else if (match) {
    ...
    case "get-task-summary": {
      const content = readFile("tasks.md");
      if (!content) return { content: [{ type: "text", text: "tasks.md does not exist yet." }] };
      const tasks = parseTasks(content);
    ...
    /* ---- Hook management ---- */
    case "create-hook": {
      const { name: hookName, event, description, instructions } = args as {
        name: string; event: HookEvent; description: string; instructions: string;
      };
      ensureDir(hooksDir());
      const content = [
        `# Hook: ${hookName}`,
    ...
    case "delete-hook": {
      const { name: hookName } = args as { name: string };
      const fp = hookFilePath(hookName);
      if (!fs.existsSync(fp)) {
    ...
    case "run-hooks": {
      const { event } = args as { event: HookEvent };
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  const file = uri.replace("spec://", "") as SpecFile;
  if (!VALID_FILES.includes(file)) throw new Error(`Unknown resource: ${uri}`);
